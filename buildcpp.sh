...
for ABI in $ABIS; do
	case "$ABI" in
		"armeabi-v7a")
			...
			ABI_FLAGS="-std=c++17 -march=armv7-a -mfloat-abi=softfp -mfpu=vfpv3-d16"
			;;

		"arm64-v8a")
			CCTYPE="aarch64-linux-android"
			ABI_FLAGS="-std=c++17 -target aarch64 -mfix-cortex-a53-835769"
			;;
	...
	echo "\nCompile project src/*.cpp to libmain.so"
	$CC src/cpp/arkanoid.cpp -o android/build/lib/$ABI/libmain.so -shared \
		$INCLUDES -I$TOOLCHAIN/sysroot/usr/include/$CCTYPE $FLAGS $ABI_FLAGS \
...
for ABI in $ABIS; do
    ...
    $BUILD_TOOLS/aapt add ../../android/game.apk lib/$ABI/libc++_shared.so
