...
void InitGame(void)
{
    ...
    PlaySound(fxOn);
    brickColor = (Color){255, 255, 255, 255};
    // Sound fxOgg = LoadSound("resources/target.ogg");
#ifdef _ANDROID_
    angle = InitAndroidSensor(GAMEVECTOR, 10000);
#endif
    Vector2 bsize;
    bsize.x = screenWidth / BRICKS_PER_LINE;
    bsize.y = 20;
    brickSize = bsize;
    //p = new Player::Player();

    //(Vector2){GetScreenWidth() / BRICKS_PER_LINE, (float)40.0f};

    // brickSize = (Vector2){GetScreenWidth() / BRICKS_PER_LINE, (float)40.0f};

    // Vector2 v = new Vector2(10.0f,20.0f);


    // Initialize player
    p.position = (Vector2){screenWidth / 2, screenHeight * 7 / 8};
    p.size = (Vector2){screenWidth / 5, 20};
...
void UpdateGame(void)
{
    if (!gameOver)
    {
        ...
        if (!pause)
        {
            ...
            if ((p.position.x - p.size.x / 2) <= 0)
                p.position.x = p.size.x / 2;
            if (IsKeyDown(KEY_RIGHT))
                p.position.x += 5;
            if ((p.position.x + p.size.x / 2) >= screenWidth)
                p.position.x = screenWidth - p.size.x / 2;
            ...
            {
                ball.position = (Vector2){p.position.x, screenHeight * 7 / 8 - 30};
            }
            ...
            if (CheckCollisionCircleRec(ball.position, ball.radius,
                                        (Rectangle){p.position.x - p.size.x / 2, p.position.y - p.size.y / 2, p.size.x, p.size.y}))
            {
                if (ball.speed.y > 0)
                {
                    ball.speed.y *= -1;
                    ball.speed.x = (ball.position.x - p.position.x) / (p.size.x / 2) * 5;
                    PlaySound(fxOn);
                }
            }
...
void DrawGame(void)
{
    ...
    if (!gameOver)
    {
        ...
        DrawCircleV(ball.position, ball.radius-2, RED);
        ...
        for (int i = 0; i < LINES_OF_BRICKS; i++)
        {
            for (int j = 0; j < BRICKS_PER_LINE; j++)
            {
                ...
                {
                    if ((i + j) % 2 == 0){
                        brickColor.r = 255 * i / LINES_OF_BRICKS;
                        brickColor.b = 255 * i / LINES_OF_BRICKS;
                        brickColor.g = 255 * j / BRICKS_PER_LINE;
